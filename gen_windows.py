...
def main():
    ...
    for py_file in glob.glob("**/*.py", recursive=True):
        ...
        normalized = py_file.replace("\\", "/")  # ✅ normalize Windows paths

        with open(py_file, "r", encoding="utf-8") as f:
            content = f.read()
            files[normalized] = content
            file_hashes[normalized] = hashlib.md5(content.encode()).hexdigest()
        print(f"Including {normalized}")
    ...
    for pattern in ["**/*.txt", "**/*.json", "**/*.yaml", "**/*.yml"]:
        for file_path in glob.glob(pattern, recursive=True):
            ...
            normalized = file_path.replace("\\", "/")  # ✅ normalize these too
